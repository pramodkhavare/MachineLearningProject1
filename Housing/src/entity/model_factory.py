...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            #Loggig of Previus data
            logging.info(f"Previous Base Accuracy: [{base_accuracy}]")

            
            #logging all important metric
            logging.info(f"{'>>'*15} Score of Model: [{type(model).__name__}] {'<<'*15}")
            logging.info(f"Train Score\t\t Test Score\t\t Model Accuracy")
            logging.info(f"{train_acc}\t\t {test_acc}\t\t{model_accuracy}")

            logging.info(f"{'>>'*15} Loss of Model: [{type(model).__name__}]  {'<<'*15}")
            ...
            if model_accuracy >= base_accuracy and diff_test_train_acc < 0.05:
                ...
                logging.info(f"Acceptable model foundAcceptable model found {metric_info_artifact}. ")
            index_number += 1
            print(model_name ,index_number)

        if metric_info_artifact is None:
            logging.info(f"No model found with higher accuracy than base accuracy")
        else:
            print(f"Model IS Accepted With Accuracy :-{metric_info_artifact.model_accuracy} ")
